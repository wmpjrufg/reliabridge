...
- **Plano do artigo 3 refinado, com mudança de recorte.** A leitura do núcleo estabeleceu que o modelo estrutural é algébrico e monótono nas variáveis de projeto, do que decorrem duas consequências. A capacidade de carga de geometria fixa sai por inversão exata, porque os esforços são lineares no multiplicador do carregamento móvel e as resistências não dependem dele, de modo que regressão simbólica sobre esse rótulo aprenderia uma função já conhecida — o boneco `05_boneco_capacidade_carga.md` foi marcado como superado, com registro do que dele vira seção. E o próprio dimensionamento admite solução direta, já que a única descontinuidade é a contagem inteira de peças de `restringir_espaco`, então fixadas as duas contagens as verificações viram um sistema monótono de três incógnitas e a enumeração exaustiva resolve o problema. O marco zero do artigo passou a ser implementar esse solucionador e medi-lo contra o NSGA-II, e dele dependem a identidade do artigo e a revista. Reescritos `01_proposta.md`, `04_revistas_e_referencias.md` e o README da pasta; o material passa de cinco classes colineares para as propriedades medidas das 40 espécies, com partição por espécie. Alvo editorial padrão **Structures**, com Engineering Structures reservada ao caso em que o achado do marco zero carregue o artigo sozinho.
- **Pergunta de projeto do artigo 3 fechada na mesma discussão.** O primeiro refinamento tinha posto o solucionador direto como marco zero, o que é pergunta de método e não de projeto. A pergunta passou a ser **até onde a ponte de madeira roliça serve e o que define esse limite**, com a restrição que fecha a fronteira sendo o diâmetro comercial de tora, não uma verificação normativa. O sinal já está nos dados, o diâmetro ótimo do piloto vai de 32,8 cm em L=3 m com TB-240 a 82,0 cm em L=10 m com TB-450, contra teto de busca de 100 cm, e `diagnosticar_limites.py` já separa limite construtivo de limite de domínio. O solucionador direto virou instrumento, porque torna a grade de 24.300 casos computável em minutos contra 142 horas de NSGA-II. A campanha não precisa de sementes, que sobram só na comparação com o NSGA-II e na regressão simbólica. `01_proposta.md` reescrito com campanha, saídas, ordem de trabalho e seção "Onde paramos"; P-13, o diâmetro comercial de tora, deixou de ser detalhe de campanha e virou condição do artigo.
- **Dois bloqueadores nomeados no código.** `calcular_objetivos_restricoes_otimizacao` usa `esp_long_corr`, o espaçamento **livre**, como largura tributária da longarina e como vão do tabuleiro, quando o valor entre eixos seria `esp_long_corr + d`; no artigo 2 o desvio se cancela na comparação pareada, no artigo 3 não, porque as dimensões absolutas seriam publicadas como regra de projeto. E em `paper/ia_explicavel/scripts/dataset.py` os campos `volume_robust_mean_m3` e `g_robust_mean` estão mal nomeados, o primeiro é numericamente idêntico a `volume_nominal_m3` e o segundo é pior caso sobre a grade de diâmetro, não média — `02_dataset.md` descreve trinta perturbações aleatórias agregadas por média, que o código não faz.
