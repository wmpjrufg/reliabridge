...
em regiões de estradas vicinais do Brasil. A ponte exemplificada consiste em uma ponte rural de pequeno porte, construída predominantemente em madeira, com sistema estrutural longitudinal. Sua estrutura é composta por apoios nas margens do córrego, que recebem e transferem as cargas para o terreno, e por vigas de madeira dispostas longitudinalmente, responsáveis por vencer o vão e suportar as cargas dos veículos. Sobre essas vigas é instalado o tabuleiro, formado por peças de madeira que constituem o piso da ponte e distribuem as cargas para a estrutura principal e pranchas longitudinais sobre o tabuleiro formando o rodeiro. O conjunto também necessita de elementos de travamento e fixação, como parafusos, pregos, barras ou chapas, garantindo a estabilidade e a união entre as peças. Dessa forma, o caminho das cargas ocorre do veículo para o tabuleiro, do tabuleiro para as vigas e, posteriormente, para os apoios e o terreno.
...
\begin{figure}[htpb!]
    \centering
    \caption{Diâmetro Equivalente.}
    \label{fig:dequivalente}
    \includegraphics[width=1.2\textwidth]{figuras/dequivalentee.png}\\
    \vspace{0.1cm}
    \centering \small Fonte: Adaptado de \parencite{NBR7190-1:2022}
\end{figure}
...
As flechas instantâneas, $\delta_{\mathrm{inst},G_i,k}$ e $\delta_{\mathrm{inst},Q_j,k}$, associadas respectivamente às ações permanentes e variáveis, são calculadas com o módulo de elasticidade médio à flexão, $E_{0,\mathrm{m}}$, e não com um módulo minorado pelo coeficiente de modificação: o fator $k_{\mathrm{mod}}$ reduz a resistência característica da madeira ($k_{\mathrm{mod},1} \cdot k_{\mathrm{mod},2} \cdot X_k$, Equação \ref{eq:2.099}), não a sua rigidez.
O efeito do tempo sobre a rigidez é incorporado separadamente, por meio do coeficiente de fluência $\varphi$, que amplia as flechas instantâneas para obter as flechas finais $\delta_{\mathrm{fin},G_i,k}$ e $\delta_{\mathrm{fin},Q_j,k}$, conforme as Equações \ref{eq:flecha_fin_G} e \ref{eq:flecha_fin_Q}.
...
\vspace{0.2cm}
\small Fonte: ABNT NBR 8681:2025, para a combinação e o tipo de ação considerados.
\end{table}
