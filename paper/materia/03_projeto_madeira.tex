...
O momento fletor máximo sob a roda central resulta da Equação~\eqref{eq:mqk30}, escrita para $3\,\text{m} < L \leq 6\,\text{m}$, faixa em que a parcela de multidão simétrica $c$ (Equação~\eqref{eq:cmomento}, aplicável apenas para $L > 6\,\text{m}$) ainda não existe, pois o próprio veículo, com as rodas externas a $2a = 3{,}0$~m uma da outra, já ocupa o vão quase por inteiro. Como se mostra adiante, essa expressão corresponde ao arranjo centralizado e só coincide com a envoltória de Barré a partir de $L \approx 3{,}34$~m.
...
A posição centralizada, contudo, só é a posição de Barré enquanto as três rodas permanecem efetivamente carregando o vão. Com as rodas externas a $L/2 \pm a$ do apoio esquerdo, elas alcançam os apoios quando $L = 2a$; a partir daí passam a ter braço nulo, ou a sair do vão, e o arranjo centralizado deixa de maximizar o momento. Para $a = 1{,}5$~m e $L = 3{,}0$~m, as rodas externas assentam exatamente sobre os apoios e a Equação~\eqref{eq:mqk30} degenera no caso de carga única centrada, $M_{qk} = PL/4$.

Nessa situação o máximo é obtido deslocando o comboio de modo que apenas duas rodas permaneçam no vão, posicionadas segundo o Teorema de Barré: o meio do vão bissecta a distância entre uma das cargas e a resultante do par. A condição fornece a posição $x = (2L-a)/4$ para a primeira roda e o momento da Equação~\eqref{eq:mqk2}, obtido maximizando $M(x) = P\,x\,(2L - 2x - a)/L$.
...
\begin{equation}
Q_{qk} = \frac{P}{L}\Big[\max(L-2h,\,0) + \max(L-2h-a,\,0) + \max(L-2h-2a,\,0)\Big] + \frac{q}{2L}\Big[\max(L-2h-3a,\,0)\Big]^{2}
\label{eq:qqkenv}
\end{equation}

Essa reformulação preserva o afastamento $2h$, inclui somente as rodas efetivamente apoiadas no vão e elimina a parcela de multidão quando não há comprimento disponível. Quando o esquema completo cabe no vão, isto é, quando $L \geq 2h + 3a$, todos os operadores $\max$ tornam-se inativos e a Equação~\eqref{eq:qqkenv} recupera algebricamente a Equação~\eqref{eq:qqk}, que permanece no texto como formulação de referência e como caso de verificação da rotina numérica. É a Equação~\eqref{eq:qqkenv} que a plataforma avalia.
