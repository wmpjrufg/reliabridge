...
Para a carga variável, o trem-tipo é posicionado sobre o vão de modo a produzir o efeito mais desfavorável em cada verificação, e essa posição de pior caso é diferente para o momento fletor, para o cortante e para a reação de apoio \parencite{CALIL2006}.

\subsubsection*{Momento fletor e flecha}

Para o momento fletor e para a flecha, o veículo é centralizado no vão: a roda central posiciona-se exatamente no meio do vão, com as duas rodas externas a uma distância $a$ dela, conforme a Figura~\ref{fig:posicao_momento_flecha}. Essa é a posição do Teorema de Barré para um grupo de cargas igualmente espaçadas cujo resultante coincide com a roda central.
...
O momento fletor máximo sob a roda central resulta da Equação~\eqref{eq:mqk30}, válida para $3\,\text{m} < L \leq 6\,\text{m}$, faixa em que a parcela de multidão simétrica $c$ (Equação~\eqref{eq:cmomento}, aplicável apenas para $L > 6\,\text{m}$) ainda não existe, pois o próprio veículo, com as rodas externas a $2a = 3{,}0$~m uma da outra, já ocupa o vão quase por inteiro.
...
Para o cortante, a Figura~\ref{fig:posicao_cortante} mostra o veículo afastado do apoio por $2h$, sendo $h$ a altura (ou o diâmetro, para seção circular) da peça: a norma admite avaliar o cortante em uma seção afastada do apoio, e não na face do apoio propriamente dita.
...
\tofill{[nota para revisão -- não integrada ao texto ainda]: a Equação~\eqref{eq:ecortante} pressupõe $L \geq 2h + 3a$, isto é, que sobra vão suficiente, depois de afastar o veículo por $2h$ do apoio, para acomodar as três rodas e ainda alguma parcela de multidão. Essa condição, folgada para vigas usuais, deixa de valer justamente na faixa de interesse deste trabalho: longarinas roliças de diâmetro grande frente ao vão (até $d \approx 1{,}5$~m para vãos de $3$ a $6$~m), em que $2h + 3a$ pode superar $L$. Quando isso ocorre, $e$ torna-se negativo e a Equação~\eqref{eq:qqk} deixa de representar o posicionamento de pior caso -- ela subestima $Q_{qk}$, tanto mais quanto maior o diâmetro, exatamente o padrão observado na varredura da Seção~\ref{sec:varredura}. A plataforma precisa passar a calcular $Q_{qk}$ pela envoltória do posicionamento do veículo (rodas nas posições reais, somando-se apenas as parcelas efetivamente apoiadas no vão), e não mais pela Equação~\eqref{eq:qqk} isoladamente; esta última permanece no texto como a formulação de referência da apostila e como caso de verificação da rotina numérica, válida quando $L \geq 2h+3a$.]}
...
A verificação de flecha compara simultaneamente essa flecha total, na combinação quase permanente, com o limite normativo $L/350$ e a flecha devida isoladamente à carga variável, na combinação rara, com o limite $L/500$, ambos da Tabela 21 da \textcite{NBR7190-1:2022}, conforme a Equação~\eqref{eq:verflecha}.

\begin{equation}
\max\left(\frac{\delta_{\mathrm{fin}}}{L/350},\ \frac{\delta_{qk}}{L/500}\right) \leq 1
\label{eq:verflecha}
\end{equation}
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.55\textwidth]{figuras/posicionamento_roda_tabuleiro.png}
