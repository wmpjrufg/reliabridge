...
O trecho útil da fronteira para o projetista concentra-se, assim, na primeira metade do intervalo de volumes. Já em 7{,}90~m\textsuperscript{3}, cerca de um terço do caminho entre os extremos, a flecha cai para 7{,}2\% do limite. Além desse ponto, o retorno marginal despenca: de $V = 8{,}82$~m\textsuperscript{3} para $V = 13{,}21$~m\textsuperscript{3} (acréscimo de 49{,}5\%), a flecha cai de apenas 5{,}4\% para 2{,}1\% do limite, um ganho de pouco mais de três pontos percentuais sobre uma margem já folgada. Aumentar o volume além do primeiro terço da fronteira, portanto, deixa de se justificar na prática.
...
A faixa admissível de $h$ e de $d$ adotada nesta matriz (Tabela~\ref{tab:intervalos}) é relativamente estreita e desloca a solução de referência para uma região do espaço de busca sensível à perturbação geométrica de $\pm\rho$ no diâmetro.
No valor de projeto $\rho = 5\%$ adotado no restante da matriz, o custo de robustez já responde por 6{,}0\% de acréscimo de volume no extremo econômico da fronteira, e cresce de forma consistente até os 26{,}9\% observados no extremo superior da grade estendida.
Como cada nível de $\rho$ ainda corresponde a uma única execução do NSGA-II, sem repetição de semente, os valores de $\Delta$ carregam alguma incerteza de convergência residual, mas a monotonicidade estrita observada em toda a grade de cinco pontos indica que o efeito supera essa incerteza.
...
O valor $\rho = 5\%$ permanece adotado como padrão em toda a matriz (Seção~\ref{sec:varredura}) por consistência com a literatura sobre tolerâncias dimensionais de peças roliças e por representar, à luz deste experimento, um compromisso razoável entre um acréscimo de custo de cerca de 6\% no extremo econômico e a proteção contra a violação de estados limites sob o desvio dimensional natural da madeira roliça.
Esse custo cresce de forma previsível com a tolerância de projeto exigida, o que dá ao projetista uma base quantitativa, e não apenas qualitativa, para a escolha de $\rho$ em obras que demandem tolerância dimensional diferente da aqui adotada.
...
Os valores finais do indicador, o \textit{spacing} da fronteira e a geração de estabilização das vinte células da matriz são reunidos na Tabela~\ref{tab:hv} do Apêndice~\ref{ap:hv}, pois o mesmo par $N_{gen}$, $N_{pop}$ foi imposto a toda a matriz e a verificação de convergência só é conclusiva se valer para a célula mais difícil, e não apenas para a de referência.
...
A curva da Figura~\ref{fig:convergencia_hv} apresenta o comportamento típico de uma execução bem dimensionada, com crescimento acentuado nas primeiras vinte gerações e refinamento assintótico depois disso.
Para a célula C-13, o hipervolume atinge 1{,}017, com \textit{spacing} de 0{,}0173, e estabiliza na geração 40.
Nas vinte células da matriz, a estabilização ocorre entre as gerações 21 e 153, com média de 80{,}2, e a célula mais lenta, C-09, ainda converge com folga de 147 gerações sobre as 300 executadas, o que confirma $N_{gen} = 300$ para toda a matriz (Tabela~\ref{tab:hv}, Apêndice~\ref{ap:hv}).
O hipervolume final varia entre 0{,}890 e 1{,}105, entre 74\% e 91\% do máximo teórico de 1{,}21, e decresce com o vão dentro de cada classe, refletindo fronteiras objetivamente mais curtas sob restrições mais severas, e não pior convergência.
O \textit{spacing} varia pouco, entre 0{,}0136 e 0{,}0203, com média de 0{,}0167, e não se correlaciona com o vão ou a classe, o que indica fronteiras igualmente bem distribuídas em toda a matriz, condição necessária para que a Tabela~\ref{tab:resultados_matriz} seja comparável linha a linha.
...
O espaçamento entre longarinas é o vão de flexão local da prancha sob a roda do trem tipo, e o momento solicitante cresce com o quadrado desse vão, ao passo que a resistência da prancha cresce apenas com $b_w h^2$.
...
A flexão domina integralmente a matriz. Quinze das vinte células são governadas pela flexão do tabuleiro e as cinco restantes pela flexão da longarina, sem nenhuma célula governada pela flecha.
...
A proximidade da flecha da longarina ao limite nas três células mais severas, ainda que sem chegar a governar, já sinaliza a transição de resistência para rigidez dentro da própria faixa de 3 a 6~m estudada, o que qualifica, sem invalidar, o campo de validade dos ábacos da seção seguinte.
