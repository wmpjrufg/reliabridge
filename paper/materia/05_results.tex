...
\begin{fillblock}
\noindent\textbf{[PENDENTE -- regenerar o lote antes de submeter]} As vinte células da matriz (Tabela~\ref{tab:matriz}), a varredura de robustez da célula C-13, as figuras e o Apêndice~\ref{ap:demonstracao} desta seção ainda refletem a execução antiga, com classe de carregamento de \emph{média duração}, $\gamma_g = 1{,}35$, $\varphi = 0{,}60$, e um erro de implementação em que a carga de multidão $p_{qk}$ (kPa) entrava direto nas Equações de $M_{qk}$ e $Q_{qk}$ sem ser convertida para carga linear (kN/m) pelo espaçamento entre longarinas. O código e a Seção~\ref{sec:ponte_madeira} já foram corrigidos (classe de curta duração, $\gamma_g = 1{,}30$, $\varphi = 0{,}80$ e a conversão $p_{qk,long} = p_{qk} \cdot esp_{long,corr}$), e a Tabela~\ref{tab:parametros} abaixo já reflete esses valores. É necessário rodar novamente \texttt{rodar\_lote.py} e atualizar a Tabela~\ref{tab:matriz}, a Tabela~\ref{tab:solucoes_ref}, as figuras e o Apêndice~\ref{ap:demonstracao} com os novos resultados antes da submissão.
\end{fillblock}
