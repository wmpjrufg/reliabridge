...
Os dois objetivos variam, portanto, em escalas distintas: enquanto o volume pouco mais que duplica (2{,}6 vezes), a flecha adimensional cai cerca de treze vezes.
...
O trecho útil da fronteira para o projetista concentra-se, assim, na primeira metade do intervalo de volumes: já em 7{,}90~m\textsuperscript{3}, cerca de um terço do caminho entre os extremos, a flecha cai para 7{,}2\% do limite.
...
A leitura de projeto que decorre disso é relevante: percorrer a fronteira significa engrossar a longarina, que rapidamente ganha folga, ao passo que o tabuleiro permanece encostado no seu limite ao longo de toda a extensão do espaço de compromisso.
...
A grade foi estendida até $\rho = 20\%$, bem além do valor de projeto de 5\% adotado no restante da matriz, especificamente para verificar se a tendência observada nos níveis mais baixos se sustenta em um regime de perturbação mais severo, de modo a caracterizar a relação entre $\rho$ e o custo de volume ao longo de uma faixa mais ampla do que a estritamente necessária ao projeto.
Seguindo recomendação do orientador desta pesquisa, a comparação é feita em um único ponto de cada fronteira, a solução de menor volume (o extremo de interesse imediato para o anteprojeto), e não por faixas pareadas de flecha adimensional, leitura descartada por ser sensível ao ruído de uma única execução independente do NSGA-II por nível de $\rho$.
...
A Figura~\ref{fig:custo_robustez} mostra as cinco fronteiras deslocando-se de forma visível e ordenada para o interior do domínio viável à medida que $\rho$ cresce, um comportamento bem mais nítido do que o observado em versões anteriores desta matriz, nas quais o deslocamento entre níveis de $\rho$ se confundia com o ruído de execuções independentes.
...
A taxa marginal de custo cresce discretamente com $\rho$, de cerca de 1{,}2\% de volume adicional por ponto percentual de $\rho$ entre 0 e 5\%, a cerca de 1{,}3\% a 1{,}4\% por ponto percentual nos níveis mais altos da grade: uma leve convexidade, e não uma relação estritamente linear, mas sem indício de saturação dentro da faixa estudada.

Este resultado contrasta com o de uma versão anterior deste experimento, que não detectava custo de robustez mensurável para $\rho$ até 10\%: a diferença é atribuível à revisão dos limites de busca das variáveis de projeto (Tabela~\ref{tab:intervalos}), que estreitou substancialmente a faixa admissível de $h$ e de $d$ em relação à versão anterior e desloca a solução de referência para uma região do espaço de busca mais sensível à perturbação geométrica de $\pm\rho$ no diâmetro.
A leitura honesta é que o custo de robustez, ao contrário do que a versão anterior sugeria, é real e mensurável nesta célula: no valor de projeto $\rho = 5\%$ adotado no restante da matriz, ele já responde por 6{,}0\% de acréscimo de volume no extremo econômico da fronteira, e cresce de forma consistente até os 26{,}9\% observados no extremo superior da grade estendida.
Como cada nível de $\rho$ ainda corresponde a uma única execução do NSGA-II, sem repetição de semente, os valores de $\Delta$ carregam alguma incerteza de convergência residual, mas a monotonicidade estrita observada em toda a grade de cinco pontos, ausente na versão anterior, indica que o efeito já supera essa incerteza, ao contrário do que ocorria antes.

As cinco execuções, rodadas sequencialmente na mesma máquina, consumiram 1{,}6~s ($\rho = 0\%$), 3{,}0~s ($\rho = 5\%$), 3{,}0~s ($\rho = 10\%$), 3{,}1~s ($\rho = 15\%$) e 3{,}0~s ($\rho = 20\%$) de processamento.
A diferença entre o caso determinístico e os demais decorre integralmente de $N_c$ colapsar para uma única avaliação quando $\rho = 0$ (Seção~\ref{sec:robustez}), contra os $N_c = 5$ pontos da grade de robustez do diâmetro nos demais níveis: uma vez ativado o mecanismo de robustez, o tempo de processamento é praticamente insensível à magnitude de $\rho$, consistente com o custo computacional escalar com o número de pontos da grade de robustez avaliados por indivíduo, e não com a amplitude da perturbação.

O valor $\rho = 5\%$ permanece adotado como padrão em toda a matriz (Seção~\ref{sec:varredura}) por consistência com a literatura sobre tolerâncias dimensionais de peças roliças e por representar, à luz deste experimento, um compromisso razoável: um acréscimo de custo de cerca de 6\% no extremo econômico, em troca de proteção contra a violação de estados limites sob o desvio dimensional natural da madeira roliça.
O que a Tabela~\ref{tab:custo_robustez} estabelece é que esse custo cresce de forma previsível com a tolerância de projeto exigida, o que dá ao projetista uma base quantitativa, e não apenas qualitativa, para a escolha de $\rho$ em obras que demandem tolerância dimensional diferente da aqui adotada.
...
\midrule
C-01 & 3{,}0 / D20 & 1{,}018 & 0{,}0150 & 124 \\
C-02 & 3{,}0 / D30 & 1{,}041 & 0{,}0160 & 68 \\
C-03 & 3{,}0 / D40 & 1{,}070 & 0{,}0169 & 69 \\
C-04 & 3{,}0 / D50 & 1{,}096 & 0{,}0171 & 50 \\
C-05 & 3{,}0 / D60 & 1{,}105 & 0{,}0158 & 97 \\
\midrule
C-06 & 4{,}0 / D20 & 0{,}972 & 0{,}0181 & 95 \\
C-07 & 4{,}0 / D30 & 1{,}023 & 0{,}0165 & 73 \\
C-08 & 4{,}0 / D40 & 1{,}045 & 0{,}0158 & 110 \\
C-09 & 4{,}0 / D50 & 1{,}040 & 0{,}0184 & 153 \\
C-10 & 4{,}0 / D60 & 1{,}082 & 0{,}0146 & 47 \\
\midrule
C-11 & 5{,}0 / D20 & 0{,}907 & 0{,}0136 & 113 \\
C-12 & 5{,}0 / D30 & 0{,}991 & 0{,}0203 & 98 \\
\textbf{C-13} & 5{,}0 / D40 & 1{,}017 & 0{,}0173 & 40 \\
C-14 & 5{,}0 / D50 & 1{,}042 & 0{,}0195 & 21 \\
C-15 & 5{,}0 / D60 & 1{,}036 & 0{,}0166 & 41 \\
\midrule
C-16 & 6{,}0 / D20 & 0{,}890 & 0{,}0173 & 119 \\
C-17 & 6{,}0 / D30 & 0{,}964 & 0{,}0156 & 53 \\
C-18 & 6{,}0 / D40 & 1{,}024 & 0{,}0182 & 101 \\
C-19 & 6{,}0 / D50 & 1{,}020 & 0{,}0174 & 90 \\
C-20 & 6{,}0 / D60 & 1{,}024 & 0{,}0148 & 42 \\
\bottomrule
\end{tabular*}
\begin{tablenotes}
\footnotesize
...
Para a célula C-13, o hipervolume atinge 1{,}017 e permanece dentro de 1\% desse valor a partir da geração 40, isto é, em pouco mais de um décimo das 300 gerações executadas.
Estendendo a leitura a toda a matriz, a estabilização ocorre entre as gerações 21 e 153, com média de 80{,}2.
A célula mais lenta a convergir, a C-09, estabiliza na geração 153, ainda com folga de 147 gerações.
...
O hipervolume final varia entre 0{,}890 e 1{,}105, ou seja, entre 74\% e 91\% do máximo teórico, e decresce sistematicamente com o vão dentro de cada classe: as células de 6~m apresentam os menores valores, e as de 3~m os maiores.
...
O \textit{spacing} varia pouco em toda a matriz, entre 0{,}0136 e 0{,}0203, com média de 0{,}0167, e não apresenta correlação com o vão ou com a classe.
...
O algoritmo converge para esse valor independentemente do ponto da fronteira escolhido pelo projetista, o que tem consequência prática ainda mais marcada do que em versões anteriores da matriz: $b_w \approx 45{,}5$~cm pode ser fixado a priori no anteprojeto, sem penalidade de desempenho, reduzindo de cinco para quatro a dimensionalidade efetiva da decisão.
As duas variáveis restantes ocupam posições intermediárias, mas com leitura distinta entre si: 17{,}9\% de coeficiente de variação para a altura da prancha (4{,}4~cm de amplitude), moderada como nas versões anteriores, e 33{,}1\% para o espaçamento entre peças do tabuleiro (2{,}8~cm de amplitude), este bem mais disperso do que em versões anteriores desta matriz, e desta vez aproximando-se do mínimo admissível de 2~cm em pelo menos um ponto da fronteira (mínimo observado de 2{,}18~cm, a apenas 0{,}18~cm do piso do domínio de busca).
...
A hipótese de inviabilidade nas classes de menor resistência sob os maiores vãos, plausível a priori, não se confirmou: a célula C-16 ($L = 6{,}0$~m, D20), a mais desfavorável da matriz, converge para uma solução viável com longarinas de 66{,}7~cm de diâmetro.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
A flexão domina integralmente a matriz nesta versão: quinze das vinte células são governadas pela flexão do tabuleiro e as cinco restantes pela flexão da longarina, sem nenhuma célula governada pela flecha.
...
A verificação de flecha da longarina ($g_3$), embora nunca governe, chega extremamente perto do limite em três células de vão e classe mais severos: utilização de 99{,}21\% na C-15 ($L = 5{,}0$~m, D60), 99{,}90\% na C-19 ($L = 6{,}0$~m, D50) e 99{,}97\% na C-20 ($L = 6{,}0$~m, D60), sempre a poucos centésimos de ponto percentual atrás da flexão do tabuleiro.
...
A consequência prática permanece, em essência, a mesma, mas com uma ressalva que se acentuou em relação à versão anterior da matriz.
...
A proximidade da flecha da longarina ao limite nas três células mais severas, ainda que sem chegar a governar nesta versão da matriz, já sinaliza a transição de resistência para rigidez dentro da própria faixa de 3 a 6~m estudada, o que qualifica, sem invalidar, o campo de validade dos ábacos da seção seguinte.
Cabe notar, por fim, que o tabuleiro governa quinze das vinte células e permanece a restrição ativa em toda a fronteira da célula de referência: ele não é um elemento secundário do dimensionamento, e merece no anteprojeto a mesma atenção dedicada às longarinas.
...
A Figura~\ref{fig:abaco_volume_m2} permite ainda comparar a eficiência entre vãos, ao normalizar o consumo pela área de tabuleiro, e mostra que o consumo unitário varia de 0{,}129~m\textsuperscript{3}/m\textsuperscript{2} no vão de 3~m com D60 a 0{,}370~m\textsuperscript{3}/m\textsuperscript{2} no vão de 6~m com D20.
