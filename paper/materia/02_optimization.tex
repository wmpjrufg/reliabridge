...
O procedimento também examinou a influência das tensões decorrentes da sequência construtiva.
...
Uma vertente distinta explorou a otimização topológica não como instrumento de redução de custo, mas como ferramenta de projeto conceitual voltada à expressão arquitetônica das pontes.
\textcite{vanBaalen2017} investigou o uso da otimização topológica na concepção de passarelas de pedestres, buscando conciliar desempenho estrutural e apelo estético das formas resultantes, em linha com a valorização crescente da expressão arquitetônica de pontes observada desde meados da década de 1990.
Essa aplicação ilustra o potencial da otimização topológica como ferramenta de pré-dimensionamento conceitual, ainda que dissociada, nesse caso, da otimização de seção que constitui o foco do presente trabalho.
...
Em uma direção complementar, voltada à fase conceitual do projeto, \textcite{Fiore2020} propuseram uma metodologia de três etapas, compreendendo condicionantes de contorno e sistema estrutural, durabilidade e níveis de vibração, para orientar decisões de forma, durabilidade e estrutura em passarelas de madeira, com vistas à extensão de sua vida útil. A proposta foi aplicada ao projeto de uma passarela de pedestres e ciclistas na Itália, evidenciando que, mesmo fora do escopo estrito da otimização, a literatura sobre pontes de madeira já reconhece a necessidade de tratar conjuntamente aspectos estruturais e de durabilidade desde as etapas iniciais de concepção.
...
Semente aleatória & 1 \\
Pontos da grade de robustez, $N_c$ & 5 \\
...
A perturbação é aplicada exclusivamente ao diâmetro da longarina, $d$: é a única das cinco variáveis de projeto com justificativa física direta, pois a madeira roliça possui variabilidade natural de diâmetro relevante para o comportamento estrutural, ao passo que perturbar os espaçamentos ($n_{long}$, $n_{tab}$) não altera o resultado -- o valor efetivo é recalculado a partir do número inteiro de peças escolhido, não do valor perturbado -- e perturbar a seção do tabuleiro ($b_w$, $h$) introduz descontinuidades no número viável de peças, de janela mais estreita que $\pm\rho$.
Em vez de amostrar $\xi$ aleatoriamente, a perturbação segue uma grade determinística de cinco pontos, $\Lambda = \{-1,\,-\tfrac{1}{2},\,0,\,\tfrac{1}{2},\,1\}$: essa escolha elimina o viés de semente que uma amostragem aleatória de poucos pontos introduziria, ao mesmo tempo em que os cinco níveis bastam para capturar eventuais degraus de viabilidade dentro do intervalo $[-\rho,\,\rho]$, sem depender de a resposta ser monotônica.
