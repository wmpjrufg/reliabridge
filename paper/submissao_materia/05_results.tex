...
Para identificar quais variáveis de projeto mais influenciam a violação de cada restrição sob perturbação, realizou-se uma análise de sensibilidade global pelo método de Sobol \parencite{Sobol2001}, implementado 
na biblioteca \texttt{UQpy} \parencite{Olivier2020}. Os índices de efeito total $S_{T_i}$ das cinco variáveis de projeto foram estimados em relação às seis funções limite, a partir de uma amostragem independente de $N = 20\,000$ pontos no domínio de cada variável (Tabela~\ref{tab:intervalos}). Esse tamanho de amostra foi definido a partir de testes preliminares de convergência, nos quais a variação dos índices de Sobol foi inferior a 1\% ao se dobrar o tamanho da amostra.
...
A predominância do tabuleiro tem origem mecânica no acoplamento pelo espaçamento entre longarinas.
Como a seção circular é mais eficiente em volume quanto maior o diâmetro, o algoritmo concentra o material em poucas longarinas e as afasta até que o momento sob a roda, que cresce com o vão local da prancha (Equação~\eqref{eq:mqktab}), esgote a capacidade do tabuleiro.
...
Para verificar a capacidade de interpolação da Equação~\eqref{eq:pre_dimensionamento} fora do conjunto de ajuste, foram otimizadas duas configurações adicionais com vãos intermediários não pertencentes à matriz, $L = 4{,}5$~m com a classe D30 e $L = 5{,}5$~m com a classe D50, mantidos todos os demais parâmetros da Tabela~\ref{tab:parametros} e do algoritmo (Tabela~\ref{tab:nsga2}).
Nenhuma dessas duas soluções participou do ajuste dos coeficientes $a$, $b$ e $c$.
A Tabela~\ref{tab:validacao_equacao} compara o volume da solução de menor volume obtida pela otimização com o volume previsto pela equação.

\begin{table}[!ht]
\caption{Verificação da equação de pré-dimensionamento em vãos intermediários não utilizados no ajuste.\label{tab:validacao_equacao}}
\begin{threeparttable}
\begin{tabular*}{\columnwidth}{@{\extracolsep\fill}ccccc@{\extracolsep\fill}}
\toprule
$L$ (m) & Classe & $V$ otimização (m\textsuperscript{3}) & $\hat{V}$ equação (m\textsuperscript{3}) & Erro relativo \\
\midrule
...
\begin{tablenotes}
\footnotesize
\item Otimizações com $\rho = 5\%$ e os mesmos parâmetros das vinte células da matriz. O erro relativo é $(\hat{V} - V)/V$.
\end{tablenotes}
\end{threeparttable}
\end{table}

Os dois erros, de $-0{,}52\%$ e $-1{,}42\%$, situam-se na mesma ordem de grandeza do erro médio obtido sobre as células de ajuste, e os volumes otimizados ficam contidos entre os das células vizinhas da matriz de mesma classe (4{,}076 e 6{,}001~m\textsuperscript{3} para D30 entre 4 e 5~m, e 4{,}562 e 6{,}015~m\textsuperscript{3} para D50 entre 5 e 6~m).
Nas duas configurações a flexão da longarina é a verificação governante, em linha com o comportamento observado na Seção~\ref{sec:varredura}.
Esse resultado indica que a equação interpola de forma consistente entre os vãos tabulados, sem, contudo, autorizar seu uso fora do domínio investigado.