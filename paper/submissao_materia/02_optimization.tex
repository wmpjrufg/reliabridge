...
O Algoritmo~\ref{alg:nsga2} resume a lógica evolutiva e o fluxo de avaliações estruturais, incorporando a proteção contra incertezas descrita na Seção~\ref{sec:robustez}.
 ...
 \For{cada geração $t = 0$ até $N_{gen}-1$}
   \For{cada indivíduo $\mathbf{x} = (d, b_w, h, esp_{long}, esp_{tab}) \in P_t$}
...
Para um dado indivíduo $\mathbf{x} = (d,\allowbreak b_w,\allowbreak h,\allowbreak esp_{long},\allowbreak esp_{tab})$, são geradas $N_c = |\Lambda| = 5$ realizações perturbadas, denotadas por $\tilde{\mathbf{x}}^{(c)}$, conforme definido na Equação~\eqref{eq:perturbacao}.
