...
O problema é formulado com duas funções objetivo antagônicas, ambas de minimização. A primeira consiste na minimização do volume total de madeira, conforme a Equação~\eqref{eq:f1}.
...
Onde $A_{long} = \pi d^2/4$ e $A_{tab} = b_w h$ são as áreas das seções transversais da longarina e da prancha do tabuleiro, de modo que $f_1$ resulta em unidade de volume (m\textsuperscript{3}).

A segunda função objetivo consiste na minimização da utilização do estado limite de serviço, expressa pela razão entre o deslocamento total e o deslocamento limite, conforme a Equação~\eqref{eq:f2}.

\begin{equation}
f_2(\mathbf{x}) = \frac{\delta_{\mathrm{tot}}}{\delta_{\mathrm{lim}}} = \frac{\delta_{gk} + \psi_2\,(1+\varphi)\,\delta_{qk}}{L/250}
\label{eq:f2}
\end{equation}

Onde $\delta_{\mathrm{tot}}$ é o deslocamento total no meio do vão da longarina sob a combinação quase permanente das ações, e $\delta_{\mathrm{lim}} = L/250$ é o limite admissível prescrito pela ABNT NBR 7190 \parencite{NBR7190-1:2022}. A função $f_2$ é adimensional e normalizada, sendo o valor unitário correspondente exatamente ao limite normativo.

Os objetivos são antagônicos. A redução do volume exige a diminuição do diâmetro da longarina, o que reduz o momento de inércia com a quarta potência (Equação~\eqref{eq:icirc}) e intensifica os deslocamentos, elevando $f_2$. Inversamente, a redução dos deslocamentos exige o aumento da rigidez e, portanto, maior consumo de material. Nenhuma alteração nas variáveis de projeto melhora simultaneamente as duas funções, o que caracteriza um compromisso genuíno e conduz a uma fronteira de Pareto não degenerada.

Ressalta-se que as ações do trem tipo são parâmetros fixos do problema, e não variáveis de decisão: o carregamento é prescrito pela ABNT NBR 7188 \parencite{nbr7188} e define o cenário de cada caso de estudo. O efeito do trem tipo sobre a solução ótima é avaliado pela comparação entre execuções independentes da otimização, conforme apresentado na Seção~\ref{sec:comparacao}.
...
Semente aleatória & 1 \\
Número de checagens da avaliação robusta, $N_c$ & 15 \\
\bottomrule
\end{tabular*}
\end{threeparttable}
\end{table}
...
Para investigar esse efeito de forma sistemática, quatro níveis de desvio são avaliados, $\rho \in \{0\%,\ 2{,}5\%,\ 5\%,\ 10\%\}$, aplicados ao mesmo conjunto de variáveis de projeto e à mesma configuração de ponte. O nível $\rho = 0\%$ corresponde ao caso determinístico (sem robustez), empregado como referência para quantificar o acréscimo percentual de volume associado a cada nível de robustez -- o custo da robustez --, enquanto $\rho = 5\%$ é o valor adotado como caso base ao longo deste trabalho, por refletir a ordem de grandeza da variabilidade dimensional usualmente admitida para peças de madeira roliça. Os resultados dessa análise de sensibilidade são apresentados na Seção~\ref{sec:custo_robustez}.

A agregação por média, empregada nas Equações~\eqref{eq:fmed} e~\eqref{eq:gmed}, é a estratégia adotada neste trabalho. A comparação com uma formulação de pior caso, $\bar{g}_j = \max_c g_j(\tilde{\mathbf{x}}^{(c)})$, permanece como perspectiva de continuidade, discutida na Seção~\ref{sec:conclusoes}.
...
A rotina de verificação empregada em cada avaliação do NSGA-II, identificada como \textsc{VerificaEstrutura} no Algoritmo~\ref{alg:nsga2}, segue integralmente o equacionamento apresentado nesta seção. Para ilustrar a sequência completa de cálculo -- propriedades geométricas, ações permanentes e variáveis, esforços solicitantes, combinações, resistências de projeto e verificação das funções limite --, o Apêndice~\ref{ap:demonstracao} apresenta o dimensionamento manual de uma longarina circular e de uma prancha do tabuleiro, para uma solução selecionada dentre os casos de estudo da Seção~\ref{sec:casos}, permitindo conferir passo a passo os resultados emitidos pela plataforma.
