...
\toprule
$d$ (cm) & $b_w$ (cm) & $h$ (cm) & $esp_{long}$ (cm) & Área (m\textsuperscript{2}) & $\delta_{\mathrm{tot}}/(L/250)$ & $g_{\max}$ \\
\midrule
...
\begin{tablenotes}
\footnotesize
\item \tofill{[A PREENCHER] Selecionar de 5 a 7 soluções da fronteira final, cobrindo do extremo de menor área ao extremo de menor flecha adimensional. $g_{\max}$ é o maior valor entre $g_1$ a $g_6$ para a solução (deve ser $\leq 0$ para viabilidade).}
\end{tablenotes}
\end{threeparttable}
\end{table}

A fronteira eficiente é apresentada na Figura~\ref{fig:fronteira01}, relacionando a área total de madeira com a flecha total adimensional.
...
\tofill{[A PREENCHER] Descrever a tendência observada entre área e flecha adimensional ao longo da fronteira (faixa de área nos dois extremos da fronteira, em m\textsuperscript{2}).}
...
\tofill{[A PREENCHER] Descrever se o comportamento qualitativo (tendência área x flecha, superioridade frente ao Monte Carlo) se mantém em relação à Ponte 01, e destacar diferenças relevantes decorrentes do vão maior e do trem tipo mais severo.}

\subsection{Análise comparativa entre os cenários}\label{sec:comparacao}

A Tabela~\ref{tab:comparacao} resume as áreas totais de madeira obtidas para cada cenário, incluindo valores mínimos, máximos e médios ao longo da fronteira eficiente, bem como o acréscimo percentual da Ponte~02 em relação à Ponte~01.

\begin{table}[!ht]
\caption{Comparação das áreas totais de madeira para os dois casos de estudo.\label{tab:comparacao}}
\begin{threeparttable}
...
\begin{table}[!ht]
\caption{Acréscimo percentual de área em relação ao caso determinístico ($\rho = 0\%$), para três níveis pareados de flecha adimensional.\label{tab:custo_robustez}}
\begin{threeparttable}
\begin{tabular*}{\columnwidth}{@{\extracolsep\fill}lccccccc@{\extracolsep\fill}}
\toprule
$\delta_{\mathrm{tot}}/(L/250)$ & $A(0\%)$ & $A(2{,}5\%)$ & $\Delta$ & $A(5\%)$ & $\Delta$ & $A(10\%)$ & $\Delta$ \\
\midrule
...
\begin{tablenotes}
\footnotesize
\item Área em m\textsuperscript{2}; $\Delta$ = acréscimo percentual em relação a $A(0\%)$, para o mesmo nível de $\delta_{\mathrm{tot}}/(L/250)$.
\end{tablenotes}
\end{threeparttable}
\end{table}
