...
Peças roliças têm seção circular variável ao longo do comprimento. O item 9.7 da \textcite{NBR7190-1:2022} representa essa geometria por um diâmetro equivalente de cálculo, tomado na seção situada a $L/3$ da extremidade de menor diâmetro, conforme a Equação~\eqref{eq:deq}, com $d_1$ e $d_2$ os diâmetros das extremidades maior e menor:
\begin{equation}
d_{eq} = d_2 + \frac{d_1-d_2}{3}, \qquad d_{eq}\leq 1{,}5\,d_2.
\label{eq:deq}
\end{equation}
O diâmetro $d$ manipulado pela otimização é esse diâmetro equivalente, e a peça é tratada como prismática de seção uniforme. O volume de uma peça prismática equivalente não é o volume comercial da tora afilada, o que deve ser observado ao converter o indicador de consumo em quantitativo de aquisição.
...
\label{eq:volume}
\end{equation}
O indicador cobre longarinas e pranchas. Não representa custo, carbono incorporado, perdas de corte nem o volume de ligações, encontros e fundações.
