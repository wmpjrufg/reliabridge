...
\subsection{Which verification governs, and where}\label{sec:governante}

The premise of this study is that the governing verification migrates with span, and that the material property determining the design migrates with it. This premise is verified first, because everything that follows depends on it.

\begin{table}[!ht]
\caption{Governing verification of the minimum-volume solution, by span and strength class.\label{tab:governante}}
\begin{threeparttable}
\begin{tabular*}{\columnwidth}{@{\extracolsep\fill}lccccc@{\extracolsep\fill}}
\toprule
 & \multicolumn{5}{c}{Strength class} \\
\cmidrule{2-6}
...
\begin{tablenotes}
\footnotesize
\item Entries: S = shear of the stringer; B = bending of the stringer; D = deflection of the stringer; DB = bending of the deck plank. The governing verification is the one with the largest normalised limit-state function in the minimum-volume solution.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\subsection{Cost of class assignment}\label{sec:custo_enquadramento}

For each species and span, Table~\ref{tab:custo_enquadramento} reports the timber volume of the species-based design, that of the class-based design and the resulting error $\Delta V$ defined in Equation~\eqref{eq:deltaV}.

\begin{table}[!ht]
\caption{Timber volume of the species-based and class-based designs and the resulting class-assignment error, by species.\label{tab:custo_enquadramento}}
\begin{threeparttable}
\footnotesize
\setlength{\tabcolsep}{3pt}
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}llcrrrrrr@{\extracolsep{\fill}}}
\toprule
\# & Species & Class & \multicolumn{2}{c}{$L = 3.0$~m} & \multicolumn{2}{c}{$L = 5.0$~m} & \multicolumn{2}{c}{$L = 10.0$~m} \\
...
\midrule
\multicolumn{3}{l}{Mean $|\Delta V|$} & & \tofill{--\%} & & \tofill{--\%} & & \tofill{--\%} \\
\multicolumn{3}{l}{Range of $\Delta V$} & & \tofill{--\%} & & \tofill{--\%} & & \tofill{--\%} \\
\multicolumn{3}{l}{Species with $\Delta V < 0$} & & \tofill{--} & & \tofill{--} & & \tofill{--} \\
\bottomrule
\end{tabular*}
\begin{tablenotes}
\footnotesize
\item $V_{sp}$: timber volume of the species-based design (m\textsuperscript{3}). $\Delta V$: class-assignment error, Equation~\eqref{eq:deltaV}. A negative $\Delta V$ means the class-based design uses less material than the species requires, and is therefore unconservative.
...
\begin{figure}[!ht]
    ...
    \caption{Timber volume as a continuous function of the material property against the step function imposed by class-based design, one panel per span. Each point is a species designed with its measured properties; the step function is the class-based design.}
...
\subsection{Is the limitation the number of classes?}\label{sec:granularidade}

A natural objection to the preceding results is that they reflect the coarseness of a five-class system, and would be mitigated by a finer one. EN 338 defines thirteen hardwood classes, from D18 to D80, and therefore offers a direct test of that hypothesis on the same population.

\begin{table}[!ht]
\caption{Comparison of the class-assignment error under the ABNT NBR 7190-3 and EN 338 class systems.\label{tab:comparacao_normas}}
\begin{threeparttable}
\begin{tabular*}{\columnwidth}{@{\extracolsep\fill}lcc@{\extracolsep\fill}}
\toprule
 & ABNT NBR 7190-3 & EN 338 \\
\midrule
Number of hardwood classes & 5 & \tofill{13} \\
Indexing property & $f_{c0,k}$ & \tofill{$f_{m,k}$} \\
Step in tabulated $E$ between adjacent classes & 14--21\% & \tofill{--} \\
Mean within-class scatter of measured $E_{M0}$ & \tofill{--} & \tofill{--} \\
Ratio, within-class scatter to class step & \tofill{$\approx 4$} & \tofill{--} \\
Species assigned a different class by stiffness & 58\% & \tofill{--} \\
Species less stiff than their class & 35\% & \tofill{--} \\
\bottomrule
\end{tabular*}
\end{threeparttable}
\end{table}
...
\subsection{Detailed case}\label{sec:caso_detalhado}
...
\subsection{Limitations}\label{sec:limitacoes}
