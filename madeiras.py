...
def checagem_completa_longarina_madeira_flexao(
    ...
    # Empacotando todos os dados necessários para o relatório
    contexto_dados = {
        'geo': geo,
        ...
        'gamma_w': gamma_w,
        'm_gk': m_gk,
        'm_qk': m_qk / aux_ci, # Removendo o aux_ci para mostrar o valor puro se desejar, ou manter com impacto
        'v_gk': v_gk,
        'v_qk': v_qk / aux_ci,
        'res_flex': res_flex,
        'res_cis': res_cis,
        'res_flecha_var': res_flecha_var
    }

    # Gera o texto Markdown
    texto_relatorio = gerar_relatorio_md(contexto_dados)

    # Salva e Converte para PDF
    caminho_pdf = salvar_relatorio_pdf(texto_relatorio, "Relatorio_Longarina")

    return res_flex, res_cis, res_flecha_var, {}, texto_relatorio
...
def salvar_relatorio_pdf(conteudo_md: str, nome_arquivo="memoria_calculo"):
    arquivo_md = f"{nome_arquivo}.md"
    arquivo_pdf = f"{nome_arquivo}.pdf"
    
    with open(arquivo_md, "w", encoding="utf-8") as f:
        f.write(conteudo_md)
    print(f"Arquivo Markdown salvo em: {os.path.abspath(arquivo_md)}")
    
    try:
        # Verifica se pandoc existe
        subprocess.run(["pandoc", "--version"], check=True, stdout=subprocess.DEVNULL)
        
        cmd = ["pandoc", arquivo_md, "-o", arquivo_pdf, "--pdf-engine=pdflatex"]
        subprocess.run(cmd, check=True)
        print(f"PDF gerado com sucesso: {os.path.abspath(arquivo_pdf)}")
        return arquivo_pdf
    except FileNotFoundError:
        print("ERRO: Pandoc não encontrado no PATH do sistema.")
        return None
    except subprocess.CalledProcessError as e:
        print(f"ERRO ao gerar PDF (verifique se o LaTeX está instalado): {e}")
        return None
